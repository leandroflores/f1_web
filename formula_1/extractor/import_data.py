import json
import os
from formula_1.db.dependencies import DbSessionDep, _get_db_session
...
def import_constructors(year: int, db: DbSessionDep) -> dict:
    ...
    try:
        ...
        for data in response_data["ConstructorTable"]["Constructors"]:
            ...
            constructor.identifier = data.get("constructorId", "")
            constructor.name = data.get("name", "")
            constructor.nationality = data.get("nationality", "")
            constructor.url = data.get("url", "")
...
import_constructors(2015, DbSessionDep())
